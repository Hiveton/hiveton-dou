                                         ...
                                         bool selected)
{
    ...
    lv_color_t divider_color = selected ? lv_color_hex(0xFFFFFF) : lv_color_hex(0x000000);

    lv_obj_set_style_bg_color(card, bg, 0);
    lv_obj_set_style_bg_opa(card, selected ? LV_OPA_COVER : LV_OPA_TRANSP, 0);
    ...
    lv_obj_set_style_bg_color(divider, divider_color, 0);
}
...
static void ui_record_list_render_empty(void)
{
    ...
    card = ui_create_card(s_record_list_refs.list_panel, 0, 0, 528, 108, UI_SCREEN_NONE, false, 18);
...
static void ui_record_list_render_items(void)
{
    ...
    for (i = s_record_list_state.page_start; i < end_index; ++i, ++visible_index)
    {
        ...
        card = ui_create_card(s_record_list_refs.list_panel, 0, (int)(visible_index * 108U), 528, 108, UI_SCREEN_NONE, false, 0);
        ...
        meta = ui_create_label(card,
                               ...
                               false,
                               false);
        badge = ui_create_label(card,
                                ui_i18n_pick("点击播放", "Tap to play"),
                                392,
                                36,
                                112,
                                24,
        ...
        lv_obj_set_size(divider, ui_px_w(528), 1);
        ...
        selected = false;
        if (s_record_list_state.selected_index == i)
        {
            selected = true;
            ...
            recorder_service_is_playing())
        {
            ...
            lv_label_set_text(badge, ui_i18n_pick("播放中", "Playing"));
                 ...
                 playing_path[0] != '\0' &&
                 s_record_list_state.suppressed_path[0] != '\0' &&
                 strcmp(s_record_list_state.suppressed_path, s_record_list_state.files[i].path) == 0)
        {
            selected = false;
            lv_label_set_text(badge, ui_i18n_pick("点击播放", "Tap to play"));
        }

        ui_record_list_set_row_style(card, title, meta, badge, divider, selected);
...
static void ui_record_list_render(void)
{
    ...
    if (s_record_list_refs.page_label != NULL)
    {
        size_t total_pages = s_record_list_state.file_count == 0U ? 0U : (s_record_list_state.file_count + UI_RECORD_LIST_VISIBLE_COUNT - 1U) / UI_RECORD_LIST_VISIBLE_COUNT;
        size_t current_page = total_pages == 0U ? 0U : (s_record_list_state.page_start / UI_RECORD_LIST_VISIBLE_COUNT) + 1U;

        rt_snprintf(page_text, sizeof(page_text), "%lu / %lu", (unsigned long)current_page, (unsigned long)total_pages);
        lv_label_set_text(s_record_list_refs.page_label, page_text);
    }
}
...
void ui_Record_List_screen_init(void)
{
    ...
    lv_obj_set_scroll_dir(s_record_list_refs.list_panel, LV_DIR_NONE);
    lv_obj_set_scrollbar_mode(s_record_list_refs.list_panel, LV_SCROLLBAR_MODE_OFF);
    s_record_list_refs.page_label = ui_create_label(page.content,
                                                    "1 / 1",
                                                    24,
                                                    598,
                                                    120,
