...
#define BQ27220_MONITOR_POLL_MS 2000U
...
static void bq27220_monitor_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (bus != RT_NULL)
        {
            ...
            {
                xiaozhi_ui_update_charge_status((aw_charge_state == 1U) || (aw_charge_state == 2U));
            }
            else if (bq27220_monitor_read_u16(i2c_bus, BQ27220_I2C_ADDRESS, BQ27220_REG_BATTERY_STATUS, &battery_status))
            {
                xiaozhi_ui_update_charge_status((battery_status & (1U << 6)) == 0U);
            }
        }
