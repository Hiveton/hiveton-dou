...
static bool s_wakeup_request_pending = false;
static uint8_t s_standby_block_count = 0U;
...
static void sleep_manager_set_standby_pending(bool pending)
{
    rt_enter_critical();
    s_standby_pending = pending;
    rt_exit_critical();
}

static bool sleep_manager_get_standby_pending(void)
{
    bool pending;

    rt_enter_critical();
    pending = s_standby_pending;
    rt_exit_critical();

    return pending;
}

static bool sleep_manager_get_sleeping(void)
{
    bool sleeping;
...
static bool sleep_manager_standby_blocked(void)
{
    bool blocked;

    rt_enter_critical();
    blocked = s_standby_block_count > 0U;
    rt_exit_critical();

    return blocked;
}

static bool sleep_manager_try_mark_sleeping(void)
{
    bool already_sleeping;

    rt_enter_critical();
    already_sleeping = s_sleeping;
    if (!already_sleeping)
    {
        s_sleeping = true;
    }
...
static void sleep_manager_clear_sleeping(void)
{
    rt_enter_critical();
    s_sleeping = false;
    rt_exit_critical();
...
{
    rt_uint8_t brightness = sleep_manager_get_configured_brightness();
...
{
    if (!sleep_manager_try_mark_sleeping())
    {
        return;
    }

    app_watchdog_set_mode(APP_WDT_MODE_SLEEP);
    sleep_manager_set_standby_pending(false);
...
static uint32_t sleep_manager_get_report_idle_ms(void)
{
    rt_tick_t now;
    rt_tick_t elapsed;
    rt_tick_t last_activity_tick;
    bool activity_reported;

    rt_enter_critical();
    activity_reported = s_activity_reported;
    last_activity_tick = s_last_activity_tick;
    rt_exit_critical();
    ...
    now = rt_tick_get();
    elapsed = now - last_activity_tick;
    return (uint32_t)((elapsed * 1000U) / RT_TICK_PER_SECOND);
}

static bool sleep_manager_try_mark_wakeup_pending(void)
{
    bool already_pending;

    rt_enter_critical();
    already_pending = s_wakeup_request_pending;
    if (!already_pending)
    {
        s_wakeup_request_pending = true;
    }
    rt_exit_critical();
...
static void sleep_manager_clear_wakeup_pending(void)
{
    rt_enter_critical();
    s_wakeup_request_pending = false;
    rt_exit_critical();
}
...
{
    rt_enter_critical();
    if (s_standby_block_count < UINT8_MAX)
    {
        ++s_standby_block_count;
    }
    s_activity_reported = true;
    s_last_activity_tick = rt_tick_get();
    rt_exit_critical();
...
bool sleep_manager_should_enter_standby(ui_screen_id_t active_id, uint32_t inactive_ms)
{
    ...
    if (sleep_manager_standby_blocked())
    {
        return false;
    }
...
void sleep_manager_on_enter_standby(ui_screen_id_t from_screen)
{
    ...
    sleep_manager_set_standby_pending(true);
}
...
void sleep_manager_on_exit_standby(ui_screen_id_t target_screen)
{
    ...
    sleep_manager_set_standby_pending(false);
    sleep_manager_clear_wakeup_pending();
...
{
    return sleep_manager_get_sleeping();
}
...
void sleep_manager_request_wakeup(void)
{
    ...
    {
        if (!sleep_manager_try_mark_wakeup_pending())
        {
            return;
        }
        if (sleep_manager_get_sleeping() && gui_pm_is_ready())
        {
            gui_pm_fsm(GUI_PM_ACTION_WAKEUP);
        }
...
void sleep_manager_resume_sleep_cycle(void)
{
    if (!sleep_manager_get_sleeping())
    {
        if (!sleep_manager_get_standby_pending())
        {
            return;
        }
