/*
 * SPDX-FileCopyrightText: 2024-2025 SiFli Technologies(Nanjing) Co., Ltd
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include <rtthread.h>
#include <stdint.h>
#include "lwip/api.h"
#include "lwip/dns.h"
#include "lwip/apps/mqtt_priv.h"
#include "lwip/apps/mqtt.h"
#include "lwip/udp.h"
#include "lwip/tcpip.h"
#include "xiaozhi_mqtt.h"
#include "bf0_hal.h"
#include "button.h"
#include "os_adaptor.h"
#include "opus_types.h"
#include "opus_multistream.h"
#include "os_support.h"
#include "audio_server.h"
#include "drv_audprc.h"
#include "mem_section.h"
#ifdef PKG_XIAOZHI_USING_AEC
    #include "webrtc/common_audio/vad/include/webrtc_vad.h"
    #include "sifli_resample.h"
#endif
#include "bts2_app_inc.h"
#include "ble_connection_manager.h"
#include "bt_connection_manager.h"
#include "bt_env.h"
#include "xiaozhi_client_public.h"
#include "opus.h"
#include "debug.h"
#include "opus_private.h"
#include "../weather/weather.h"
#include "gui_app_pm.h"
#include "lv_display.h"
#include "ui/ui_dispatch.h"
#include "../sleep_manager.h"
#include "xiaozhi_ui.h"
#include "xiaozhi_websocket.h"
#include "xiaozhi_audio.h"
#include "network/network_mem.h"
#include "log.h"

#undef LOG_TAG
#define LOG_TAG "xz"
#define DBG_TAG "xz"
#define DBG_LVL LOG_LVL_INFO
#define XZ_THREAD_NAME "xiaozhi"
#define XZ_OPUS_STACK_SIZE (48 * 1024)  /* 语音链路栈放大，避免高峰期编码栈再次压线 */
#define XZ_EVENT_MIC_RX (1 << 0)
#define XZ_EVENT_SPK_TX (1 << 1)
#define XZ_EVENT_DOWNLINK (1 << 2)
#define XZ_EVENT_EXIT (1 << 3)
#define XZ_EVENT_SPK_CTRL (1 << 4)
#define XZ_EVENT_MIC_FLUSH (1 << 5)
#define MQTT_RECONNECT 12

#define XZ_EVENT_ALL                                                           \
    (XZ_EVENT_MIC_RX | XZ_EVENT_SPK_TX | XZ_EVENT_DOWNLINK | XZ_EVENT_EXIT |   \
     XZ_EVENT_SPK_CTRL | XZ_EVENT_MIC_FLUSH)

#define VOICE_STATE_IDLE                0
#define VOICE_STATE_WAIT_SPEAKING       1
#define VOICE_STATE_SPEAKING            2

#define VOICE_START_TIMES               (XZ_MIC_FRAME_LEN /320 * 2) /* 1 mic frames */
#define VOICE_STOP_TIMES                30

/* Ringbuffer 缓冲区大小定义 */
#define RB_VAD_CACHE_SIZE       (320 * VOICE_START_TIMES)
#define RB_OPUS_ENCODE_SIZE     (XZ_MIC_FRAME_LEN * 6)
#define XZ_SPK_WRITE_CACHE_SIZE 12000U
#define XZ_AUDIO_PSRAM_HEAP_SIZE (512 * 1024)
#define XZ_AUDIO_PSRAM_THRESHOLD 1024U
#define XZ_DOWNLINK_PACKET_HEADROOM 16U
#define XZ_DOWNLINK_QUEUE_INITIAL_SIZE 256U
#define XZ_DOWNLINK_QUEUE_GROW_ALIGN 512U
#define XZ_AUDIO_WAIT_POLL_MS 10U
#define XZ_SPK_CLOSE_DRAIN_TIMEOUT_MS 200U
#define XZ_SPK_CLOSE_CACHE_WAIT_MAX_MS 250U
#define XZ_SPK_CLOSE_CACHE_PAD_MS 20U
#define XZ_AUDIO_THREAD_EVENT_WAIT_MS 100U
#define XZ_AUDIO_THREAD_EXIT_TIMEOUT_MS 1000U

#ifndef XZ_AUDIO_HOT_LOG_ENABLE
#define XZ_AUDIO_HOT_LOG_ENABLE 0
#endif

#ifndef XZ_AUDIO_HOT_LOG_INTERVAL_MS
#define XZ_AUDIO_HOT_LOG_INTERVAL_MS 1000U
#endif

#if XZ_AUDIO_HOT_LOG_ENABLE
static rt_bool_t xz_audio_hot_log_allowed(void)
{
    static rt_tick_t last_tick = 0;
    rt_tick_t now = rt_tick_get();
    rt_tick_t interval = rt_tick_from_millisecond(XZ_AUDIO_HOT_LOG_INTERVAL_MS);

    if (last_tick == 0 || now - last_tick >= interval)
    {
        last_tick = now;
        return RT_TRUE;
    }

    return RT_FALSE;
}

#define XZ_AUDIO_HOT_LOG(...)                                                  \
    do                                                                         \
    {                                                                          \
        if (xz_audio_hot_log_allowed())                                        \
        {                                                                      \
            LOG_I(__VA_ARGS__);                                                \
        }                                                                      \
    } while (0)
#else
#define XZ_AUDIO_HOT_LOG(...)                                                  \
    do                                                                         \
    {                                                                          \
    } while (0)
#endif

/* 将 Ringbuffer 缓冲区放入 PSRAM，节省 L2 RAM (~8KB) */
#if defined(__CC_ARM) || defined(__CLANG_ARM)
L2_RET_BSS_SECT_BEGIN(g_rb_vad_cache_buf)
static uint8_t g_rb_vad_cache_buf[RB_VAD_CACHE_SIZE];
L2_RET_BSS_SECT_END
L2_RET_BSS_SECT_BEGIN(g_rb_opus_encode_buf)
static uint8_t g_rb_opus_encode_buf[RB_OPUS_ENCODE_SIZE];
L2_RET_BSS_SECT_END
L2_RET_BSS_SECT_BEGIN(g_xz_audio_psram_heap_pool)
ALIGN(4) static uint8_t g_xz_audio_psram_heap_pool[XZ_AUDIO_PSRAM_HEAP_SIZE];
L2_RET_BSS_SECT_END
#else
static uint8_t g_rb_vad_cache_buf[RB_VAD_CACHE_SIZE] L2_RET_BSS_SECT(g_rb_vad_cache_buf);
static uint8_t g_rb_opus_encode_buf[RB_OPUS_ENCODE_SIZE] L2_RET_BSS_SECT(g_rb_opus_encode_buf);
ALIGN(4) static uint8_t g_xz_audio_psram_heap_pool[XZ_AUDIO_PSRAM_HEAP_SIZE]
    L2_RET_BSS_SECT(g_xz_audio_psram_heap_pool);
#endif

#define XZ_AUDIO_VERSION            "xz_audio_verson: 1.0"

#ifdef XIAOZHI_USING_MQTT
    #define XZ_DEVICE_STATE mqtt_g_state
#else
    #define XZ_DEVICE_STATE web_g_state
#endif
extern uint8_t vad_is_enable(void);
extern uint8_t Initiate_disconnection_flag;
extern lv_obj_t *main_container;
extern lv_obj_t *standby_screen;

struct udp_pcb *udp_pcb;
xz_audio_t xz_audio;
bool g_ota_verified = true;
static volatile rt_uint8_t g_speaker_target_on = 0;
static struct rt_memheap g_xz_audio_psram_heap;
static rt_bool_t g_xz_audio_psram_heap_ready = RT_FALSE;
static rt_sem_t g_xz_mic_flush_sem = RT_NULL;
static rt_sem_t g_xz_audio_thread_exit_sem = RT_NULL;
static volatile uint32_t g_xz_downlink_drop_no_idle_count = 0;
static volatile uint32_t g_xz_downlink_drop_close_count = 0;
static volatile uint32_t g_xz_speaker_close_timeout_count = 0;
static volatile uint32_t g_xz_audio_thread_exit_timeout_count = 0;

#if defined(__CC_ARM) || defined(__CLANG_ARM)
L2_RET_BSS_SECT_BEGIN(g_xz_opus_stack) //6000地址
static uint32_t g_xz_opus_stack[XZ_OPUS_STACK_SIZE / sizeof(uint32_t)];
L2_RET_BSS_SECT_END
#else
static uint32_t
    g_xz_opus_stack[XZ_OPUS_STACK_SIZE / sizeof(uint32_t)] L2_RET_BSS_SECT(g_xz_opus_stack);
#endif

static void xz_opus_thread_entry(void *p);
static void audio_write_and_wait(xz_audio_t *thiz, uint8_t *data, uint32_t data_len);
static void xz_button_event_handler(int32_t pin, button_action_t action);
static void xz_audio_kick_downlink_decode(xz_audio_t *thiz);
static void xz_audio_encode_and_send_pcm(xz_audio_t *thiz, uint32_t pcm_bytes,
                                         rt_bool_t zero_pad_tail);
void xz_mic_open(xz_audio_t *thiz);
void xz_mic_close(xz_audio_t *thiz);
void xz_speaker_close(xz_audio_t *thiz);
void xz_speaker_open(xz_audio_t *thiz);

static xz_audio_t *xz_audio_resolve(xz_audio_t *thiz)
{
    return thiz != NULL ? thiz : &xz_audio;
}

static rt_bool_t xz_audio_heap_is_psram_ptr(void *ptr)
{
    uintptr_t addr = (uintptr_t)ptr;
    uintptr_t start = (uintptr_t)g_xz_audio_psram_heap_pool;
    uintptr_t end = start + sizeof(g_xz_audio_psram_heap_pool);

    return addr >= start && addr < end;
}

static int xz_audio_psram_heap_init(void)
{
    rt_err_t err;

    err = rt_memheap_init(&g_xz_audio_psram_heap, "xz_audio_psram",
                          g_xz_audio_psram_heap_pool,
                          sizeof(g_xz_audio_psram_heap_pool));
    if (err != RT_EOK)
    {
        LOG_E("init psram heap failed: %d", err);
        return err;
    }

    g_xz_audio_psram_heap_ready = RT_TRUE;
    LOG_I("audio psram heap ready: %u bytes", XZ_AUDIO_PSRAM_HEAP_SIZE);
    return 0;
}
INIT_PREV_EXPORT(xz_audio_psram_heap_init);

void *xz_audio_heap_malloc(rt_size_t size)
{
    void *ptr = RT_NULL;

    if (g_xz_audio_psram_heap_ready && size >= XZ_AUDIO_PSRAM_THRESHOLD)
    {
        ptr = rt_memheap_alloc(&g_xz_audio_psram_heap, size);
        if (ptr != RT_NULL)
        {
            return ptr;
        }
    }

    return rt_malloc(size);
}

void *xz_audio_heap_calloc(rt_size_t count, rt_size_t size)
{
    rt_size_t total = count * size;
    void *ptr = xz_audio_heap_malloc(total);

    if (ptr != RT_NULL)
    {
        rt_memset(ptr, 0, total);
    }

    return ptr;
}

void xz_audio_heap_free(void *ptr)
{
    if (ptr == RT_NULL)
    {
        return;
    }

    if (xz_audio_heap_is_psram_ptr(ptr))
    {
        rt_memheap_free(ptr);
        return;
    }

    rt_free(ptr);
}

static void *xz_tls_heap_malloc(rt_size_t size)
{
    void *ptr = RT_NULL;

    if (size == 0U)
    {
        return RT_NULL;
    }

    if (g_xz_audio_psram_heap_ready)
    {
        ptr = rt_memheap_alloc(&g_xz_audio_psram_heap, size);
        if (ptr != RT_NULL)
        {
            return ptr;
        }
    }

    ptr = rt_malloc(size);
    if (ptr == RT_NULL)
    {
        LOG_E("tls alloc failed size=%u", (unsigned int)size);
    }

    return ptr;
}

static void *xz_tls_heap_calloc(rt_size_t count, rt_size_t size)
{
    rt_size_t total;
    void *ptr;

    if (count == 0U || size == 0U)
    {
        return RT_NULL;
    }

    if (count > (((rt_size_t)-1) / size))
    {
        return RT_NULL;
    }

    total = count * size;
    ptr = xz_tls_heap_malloc(total);
    if (ptr != RT_NULL)
    {
        rt_memset(ptr, 0, total);
    }

    return ptr;
}

static void xz_tls_heap_free(void *ptr)
{
    xz_audio_heap_free(ptr);
}

void *hiveton_tls_malloc(size_t size)
{
    if (size > UINT32_MAX)
    {
        return RT_NULL;
    }
    return network_mem_malloc((uint32_t)size);
}

void *hiveton_tls_calloc(size_t count, size_t size)
{
    if (count > UINT32_MAX || size > UINT32_MAX)
    {
        return RT_NULL;
    }
    return network_mem_calloc((uint32_t)count, (uint32_t)size);
}

void hiveton_tls_free(void *ptr)
{
    network_mem_free(ptr);
}

void *opus_heap_malloc(uint32_t size)
{
    return xz_audio_heap_malloc((rt_size_t)size);
}

void opus_heap_free(void *ptr)
{
    xz_audio_heap_free(ptr);
}

static void xz_audio_safe_close(audio_client_t *client, const char *tag)
{
    audio_client_t handle;

    if (client == NULL || *client == NULL)
    {
        return;
    }

    handle = *client;
    *client = NULL;

    if ((((uintptr_t)handle) & (sizeof(uint32_t) - 1U)) != 0U)
    {
        LOG_E("skip invalid %s handle=%p", tag, handle);
        return;
    }

    LOG_I("close %s handle=%p", tag, handle);
    audio_close(handle);
}

static void xz_audio_release_downlink_queues(xz_audio_t *thiz)
{
    if (thiz == RT_NULL)
    {
        return;
    }

    for (int i = 0; i < XZ_DOWNLINK_QUEUE_NUM; i++)
    {
        if (thiz->downlink_queue[i].data != RT_NULL)
        {
            opus_heap_free(thiz->downlink_queue[i].data);
            thiz->downlink_queue[i].data = RT_NULL;
        }
        thiz->downlink_queue[i].size = 0;
        thiz->downlink_queue[i].data_len = 0;
    }
}

static uint32_t xz_audio_move_downlink_busy_to_idle(xz_audio_t *thiz)
{
    rt_slist_t *idle;
    uint32_t count = 0;

    if (thiz == RT_NULL)
    {
        return 0;
    }

    rt_enter_critical();
    while (1)
    {
        idle = rt_slist_first(&thiz->downlink_decode_busy);
        if (idle == RT_NULL)
        {
            break;
        }
        xz_decode_queue_t *queue = rt_container_of(idle, xz_decode_queue_t, node);
        queue->data_len = 0;
        rt_slist_remove(&thiz->downlink_decode_busy, idle);
        rt_slist_append(&thiz->downlink_decode_idle, idle);
        count++;
    }
    rt_exit_critical();

    return count;
}

static rt_bool_t xz_audio_downlink_busy_empty(xz_audio_t *thiz)
{
    rt_bool_t empty;

    if (thiz == RT_NULL)
    {
        return RT_TRUE;
    }

    rt_enter_critical();
    empty = (rt_slist_first(&thiz->downlink_decode_busy) == RT_NULL);
    rt_exit_critical();

    return empty;
}

static rt_bool_t xz_audio_wait_downlink_drain(xz_audio_t *thiz,
                                              uint32_t timeout_ms)
{
    uint32_t waited_ms = 0;

    while (!xz_audio_downlink_busy_empty(thiz))
    {
        uint32_t delay_ms;

        if (waited_ms >= timeout_ms)
        {
            return RT_FALSE;
        }

        if (thiz != RT_NULL && thiz->event != RT_NULL && thiz->is_tx_enable)
        {
            rt_event_send(thiz->event, XZ_EVENT_DOWNLINK);
        }

        delay_ms = timeout_ms - waited_ms;
        if (delay_ms > XZ_AUDIO_WAIT_POLL_MS)
        {
            delay_ms = XZ_AUDIO_WAIT_POLL_MS;
        }
        rt_thread_mdelay(delay_ms);
        waited_ms += delay_ms;
    }

    return RT_TRUE;
}

static void xz_audio_sem_drain(rt_sem_t sem)
{
    if (sem == RT_NULL)
    {
        return;
    }

    while (rt_sem_take(sem, 0) == RT_EOK)
    {
    }
}

static void xz_audio_open_cleanup(xz_audio_t *thiz)
{
    audio_client_t mic_handle;
    audio_client_t speaker_handle;
    bool shared_handle;

    if (thiz == RT_NULL)
    {
        return;
    }

    thiz->is_rx_enable = 0;
    thiz->is_tx_enable = 0;
    thiz->is_exit = 1;
    thiz->inited = 0;

    if (thiz->event != RT_NULL)
    {
        rt_event_delete(thiz->event);
        thiz->event = RT_NULL;
    }

    if (thiz->rb_opus_encode_input != RT_NULL)
    {
        rt_free(thiz->rb_opus_encode_input);
        thiz->rb_opus_encode_input = RT_NULL;
    }

#if PKG_XIAOZHI_USING_AEC
    if (thiz->rb_vad_cache != RT_NULL)
    {
        rt_free(thiz->rb_vad_cache);
        thiz->rb_vad_cache = RT_NULL;
    }
    if (thiz->resample != RT_NULL)
    {
        sifli_resample_close(thiz->resample);
        thiz->resample = RT_NULL;
    }
    if (thiz->handle != RT_NULL)
    {
        WebRtcVad_Free(thiz->handle);
        thiz->handle = RT_NULL;
    }
#endif

    xz_audio_release_downlink_queues(thiz);

    mic_handle = thiz->mic;
    speaker_handle = thiz->speaker;
    shared_handle = (speaker_handle == mic_handle);
    thiz->mic = RT_NULL;
    thiz->speaker = RT_NULL;
    xz_audio_safe_close(&mic_handle, "mic");
    if (!shared_handle)
    {
        xz_audio_safe_close(&speaker_handle, "speaker");
    }
}

void xz_audio_send(uint8_t *data, int len)
{
    uint32_t nonce[4];
    uint8_t *p_nonce = (uint8_t *)&(nonce[0]);

    if (data == RT_NULL || len <= 0 || udp_pcb == RT_NULL || g_xz_context.port == 0)
    {
        XZ_AUDIO_HOT_LOG("drop uplink audio len=%d pcb=%p port=%u", len, udp_pcb,
                         g_xz_context.port);
        return;
    }

    memcpy(p_nonce, &(g_xz_context.nonce[0]), sizeof(nonce));
    *(uint16_t *)(p_nonce + 2) = htons(len);
    *(uint32_t *)(p_nonce + 12) = htonl(++g_xz_context.local_sequence);
    // Encrypt data
    HAL_AES_init((uint32_t *)&(g_xz_context.key[0]), 16,
                 (uint32_t *)&(nonce[0]), AES_MODE_CTR);
    struct pbuf *pbuf =
        pbuf_alloc(PBUF_TRANSPORT, len + sizeof(nonce) + 32, PBUF_RAM);
    if (pbuf == RT_NULL)
    {
        XZ_AUDIO_HOT_LOG("drop uplink audio: pbuf alloc failed len=%d", len);
        return;
    }

    if (g_xz_context.port)
    {
        uint8_t *payload = (uint8_t *)pbuf->payload;
        memcpy(payload, nonce, sizeof(nonce));
        payload += sizeof(nonce);
        HAL_AES_run(AES_ENC, data, payload, len);
        LOCK_TCPIP_CORE();
        udp_sendto(udp_pcb, pbuf, &(g_xz_context.udp_addr), g_xz_context.port);
        UNLOCK_TCPIP_CORE();
    }
    pbuf_free(pbuf);
}

void xz_udp_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p,
                 const ip_addr_t *addr, u16_t port)
{
    uint8_t *data;
    uint32_t nonce[4];
    uint32_t size;
    uint32_t sequence;

    if (p == RT_NULL)
    {
        XZ_AUDIO_HOT_LOG("drop null audio pbuf");
        return;
    }

    if (addr == RT_NULL ||
        memcmp(addr, &(g_xz_context.udp_addr), sizeof(ip_addr_t)) != 0 ||
        port != g_xz_context.port)
    {
        XZ_AUDIO_HOT_LOG("drop unexpected udp audio packet");
        goto end;
    }

    if (p->payload == RT_NULL || p->len < sizeof(nonce))
    {
        XZ_AUDIO_HOT_LOG("drop short audio packet len=%u", p->len);
        goto end;
    }

    data = (uint8_t *)p->payload;
    memcpy(&(nonce[0]), data, sizeof(nonce));
    sequence = ntohl(nonce[3]);

    if (data[0] != 0x01)
    {
        XZ_AUDIO_HOT_LOG("drop invalid audio packet type=%x", data[0]);
        goto end;
    }

    if (sequence < g_xz_context.remote_sequence)
    {
        XZ_AUDIO_HOT_LOG("drop old audio packet seq=%lu expected=%lu",
                         sequence, g_xz_context.remote_sequence);
        goto end;
    }

    if (sequence != g_xz_context.remote_sequence + 1)
    {
        XZ_AUDIO_HOT_LOG("audio sequence jump seq=%lu expected=%lu", sequence,
                         g_xz_context.remote_sequence + 1);
        g_xz_context.remote_sequence = sequence;
    }
    else
    {
        g_xz_context.remote_sequence = sequence;
    }

    data += sizeof(nonce);
    size = p->len - sizeof(nonce);
    xz_audio_downlink(data, size, &nonce[0], 1);

end:
    pbuf_free(p);
}
#ifdef XIAOZHI_USING_MQT
void simulate_button_pressed()
{
     rt_kprintf("mqtt simulate_button_pressed pressed\r\n");
    if(Initiate_disconnection_flag)//蓝牙主动断开不允许mic触发
    {
        rt_kprintf("Initiate_disconnection_flag\r\n");
        return;
    }
    xz_button_event_handler(BSP_KEY1_PIN, BUTTON_PRESSED);
}
void simulate_button_released()
{
    rt_kprintf("mqtt simulate_button_released released\r\n");
    if(Initiate_disconnection_flag)
    {
        return;
    }
    xz_button_event_handler(BSP_KEY1_PIN, BUTTON_RELEASED);
}
#else 
RT_WEAK void simulate_button_pressed()
{
}
RT_WEAK void simulate_button_released()
{
}
#endif

static int mic_callback(audio_server_callback_cmt_t cmd,
                        void *callback_userdata, uint32_t reserved)
{
    // this was called every 10ms
    xz_audio_t *thiz = &xz_audio;
    if (thiz->is_rx_enable && cmd == as_callback_cmd_data_coming)
    {
        // data lengh is 320 bytes, which is 10ms for 16k samplerate
        audio_server_coming_data_t *p = (audio_server_coming_data_t *)reserved;
        if (!thiz->inited || p == RT_NULL || p->data == RT_NULL ||
            p->data_len == 0 || thiz->rb_opus_encode_input == RT_NULL)
        {
            XZ_AUDIO_HOT_LOG("drop mic frame: audio not ready");
            return 0;
        }
#ifdef PKG_XIAOZHI_USING_AEC
    if (thiz->vad_enabled) 
    {
        if (thiz->handle == RT_NULL || thiz->rb_vad_cache == RT_NULL)
        {
            XZ_AUDIO_HOT_LOG("drop mic frame: vad not ready");
            return 0;
        }
        int ret = WebRtcVad_Process(thiz->handle, 16000, (int16_t *)p->data,p->data_len / 2); //检测是否是人声 返回1是人声
        if (ret < 0)
        {
            XZ_AUDIO_HOT_LOG("drop mic frame: vad ret=%d", ret);
            return 0;
        }
        if(vad_is_enable()) //如果开起了不打断功能 1是不打断
        { 
            if (XZ_DEVICE_STATE != kDeviceStateIdle) 
            {
                return 0; // 非待命状态不处理VAD
            }
        }
        if (VOICE_STATE_IDLE == thiz->voice_state)
        {
#if !ALLOW_VAD_WHEN_SPEAKING
            if ((ret == 1))
#else
            if ((ret == 1) && (XZ_DEVICE_STATE != kDeviceStateSpeaking))
#endif
            {
                LOG_I("idle --> wait speaking");
                thiz->voice_stop_times = 0;
                thiz->voice_state = VOICE_STATE_WAIT_SPEAKING;
                thiz->voice_start_times = 0;
            }
            //return 0;
        }
        else if (VOICE_STATE_WAIT_SPEAKING == thiz->voice_state)
        {
            if(vad_is_enable()) //如果开起了不打断功能 1是不打断   
            {         
                if (XZ_DEVICE_STATE == kDeviceStateSpeaking)
                {
                    // xiaozhi is speaking, do not respond to mic input
                    LOG_I("speaking --> idle");
                    thiz->voice_start_times = 0;
                    thiz->voice_stop_times = 0;
                    thiz->voice_state = VOICE_STATE_IDLE;
                }
            }
            if (ret)
            {
                // voice
                thiz->voice_stop_times = 0;
                if (thiz->voice_start_times < VOICE_START_TIMES)
                {
                    thiz->voice_start_times++;
                    // LOG_I("wait enough voice times=%d",thiz->voice_start_times);
                    rt_ringbuffer_put(thiz->rb_vad_cache, p->data, p->data_len);
                }
                else if (thiz->voice_start_times == VOICE_START_TIMES)
                {
                        thiz->voice_start_times = 0;
                        thiz->voice_state = VOICE_STATE_SPEAKING;
                        LOG_I("call button pressed");
                        simulate_button_pressed();
                        uint8_t buf[320];
                        while (rt_ringbuffer_data_len(thiz->rb_vad_cache) >= 320)
                        {
                            rt_ringbuffer_get(thiz->rb_vad_cache, buf, 320);
                            rt_ringbuffer_put(thiz->rb_opus_encode_input, buf, 320);
                        }
                        rt_ringbuffer_put(thiz->rb_opus_encode_input, p->data, p->data_len);
                        rt_ringbuffer_reset(thiz->rb_vad_cache);
                }
            }
            else
            { // not voice
                LOG_I("wait speaking --> idle");
                thiz->voice_start_times = 0;
                thiz->voice_stop_times = 0;
                thiz->voice_state = VOICE_STATE_IDLE;
                rt_ringbuffer_reset(thiz->rb_vad_cache);
            }
            return 0;
        }
        else if (VOICE_STATE_SPEAKING == thiz->voice_state)
        {
            if (ret == 1)
            {
                XZ_AUDIO_HOT_LOG("vad speaking");
                thiz->voice_stop_times = 0;
            }
            else
            {
                XZ_AUDIO_HOT_LOG("vad not voice");
                if (thiz->voice_stop_times < VOICE_STOP_TIMES)
                {
                    thiz->voice_stop_times++;
                    XZ_AUDIO_HOT_LOG("wait no voice times=%d",
                                     thiz->voice_stop_times);
                }
                if (thiz->voice_stop_times == VOICE_STOP_TIMES)
                {
                    LOG_I("speaking --> idle");
                    thiz->voice_stop_times = 0;
                    thiz->voice_start_times = 0;
                    thiz->voice_state = VOICE_STATE_IDLE;
                    LOG_I("call button released");
                    simulate_button_released();
                    return 0;
                }
                
            }
        }
        else
        {
            XZ_AUDIO_HOT_LOG("reset invalid voice state=%d", thiz->voice_state);
            thiz->voice_start_times = 0;
            thiz->voice_stop_times = 0;
            thiz->voice_state = VOICE_STATE_IDLE;
            rt_ringbuffer_reset(thiz->rb_vad_cache);
            return 0;
        }
    }
#endif
        rt_ringbuffer_put(thiz->rb_opus_encode_input, p->data, p->data_len);
        thiz->mic_rx_count += 320;

        if (thiz->mic_rx_count >= XZ_MIC_FRAME_LEN)
        {
            thiz->mic_rx_count = 0;
            rt_event_send(thiz->event, XZ_EVENT_MIC_RX);
        }
    }
    return 0;
}

void xz_mic(int on)
{
    // Sample rate is 16K, each packet should be 60ms
    xz_audio_t *thiz = &xz_audio;
    if (on)
    {
        xz_mic_open(thiz);
    }
    else
    {
        xz_mic_close(thiz);
    }
}

void xz_speaker(int on)
{
    // Sample rate is g_xz_context.sample_rate(typical 24K),
    // each packet should be g_xz_context.frame_duration(typical 60ms)
    xz_audio_t *thiz = &xz_audio;

    g_speaker_target_on = on ? 1U : 0U;

    if (thiz->inited && thiz->event != RT_NULL &&
        rt_thread_self() != &thiz->thread)
    {
        rt_event_send(thiz->event, XZ_EVENT_SPK_CTRL);
        return;
    }

    if (on)
    {
        xz_speaker_open(thiz);
    }
    else
    {
        xz_speaker_close(thiz);
    }
}

#ifdef XIAOZHI_USING_MQTT
static void xz_button_event_handler(int32_t pin, button_action_t action)
{
    ui_dispatch_request_activity();
    sleep_manager_request_wakeup();
    rt_kprintf("in mqtt button handle2\n");
    // 如果当前处于KWS模式，则退出KWS模式
    if (g_kws_running) 
    {  
        rt_kprintf("KWS exit\n");
        g_kws_force_exit = 1;
    }
    static button_action_t last_action = BUTTON_RELEASED;
    rt_kprintf("button(%d) %d:", pin, action);
    if (last_action == action)
    {
        return;
    }
    last_action = action;

        if (action == BUTTON_PRESSED)
        {
            
            rt_kprintf("pressed\r\n");
            rt_kprintf("mqtt按键->对话\n");
            if (ui_dispatch_get_active_screen() == UI_SCREEN_HOME)
            {
                ui_switch_to_xiaozhi_screen();
            }
            if (mqtt_g_state == kDeviceStateUnknown) // goodby唤醒
            {
                mqtt_hello(&g_xz_context);
            }
            if (mqtt_g_state == kDeviceStateSpeaking)
            {
                mqtt_g_state = kDeviceStateListening;
            }

            xiaozhi_ui_chat_status("聆听中...");
        }
        else if (action == BUTTON_RELEASED)
        {
            rt_kprintf("released\r\n");
            xiaozhi_ui_chat_status("待命中...");
        }
    
}

void xz_mqtt_button_init(void)
{
    extern void app_buttons_init(void);
    app_buttons_init();
}

void xz_audio_init()
{
    rt_kprintf("xz_audio_init\n");
    rt_kprintf("exit sniff mode\n");
    // bt_interface_exit_sniff_mode(
    //     (unsigned char *)&g_bt_app_env.bd_addr); // exit sniff mode
    // bt_interface_wr_link_policy_setting(
    //     (unsigned char *)&g_bt_app_env.bd_addr,
    //     BT_NOTIFY_LINK_POLICY_ROLE_SWITCH); // close role switch
    if (udp_pcb)
    {
        udp_remove(udp_pcb);
        udp_pcb = NULL;
    }
    {
        int volume = audio_server_get_private_volume(AUDIO_TYPE_LOCAL_MUSIC);
        if (volume < 12)
        {
            audio_server_set_private_volume(AUDIO_TYPE_LOCAL_MUSIC, 12);
        }
    }
    xz_audio_decoder_encoder_open(0);
    if (!xz_audio.inited)
    {
        LOG_E("xz audio open failed");
        return;
    }

    xz_mqtt_button_init();
    udp_pcb = udp_new();
    if (udp_pcb == RT_NULL)
    {
        LOG_E("udp_new failed");
        return;
    }
    g_xz_context.local_sequence = 0;
    g_xz_context.remote_sequence = 0;
    udp_recv(udp_pcb, xz_udp_recv, NULL);
}

#endif

#ifndef XIAOZHI_USING_MQTT
void xz_audio_init(void)
{
    rt_kprintf("xz_audio_init\n");
    {
        int volume = audio_server_get_private_volume(AUDIO_TYPE_LOCAL_MUSIC);
        if (volume < 12)
        {
            audio_server_set_private_volume(AUDIO_TYPE_LOCAL_MUSIC, 12);
        }
    }
    xz_audio_decoder_encoder_open(0);
    if (!xz_audio.inited)
    {
        LOG_E("xz audio open failed");
        return;
    }
}
#endif

bool xz_audio_is_inited(void)
{
    return xz_audio.inited != 0;
}
static void audio_write_and_wait(xz_audio_t *thiz, uint8_t *data,
                                 uint32_t data_len)
{
    int ret;
#if PKG_XIAOZHI_USING_AEC
    uint32_t bytes;
    if (thiz == RT_NULL || thiz->speaker == RT_NULL || thiz->resample == RT_NULL ||
        data == RT_NULL || data_len == 0)
    {
        XZ_AUDIO_HOT_LOG("drop speaker frame: audio not ready");
        return;
    }
    bytes =
        sifli_resample_process(thiz->resample, (int16_t *)data, data_len, 0);
    if (bytes == 0)
    {
        XZ_AUDIO_HOT_LOG("drop speaker frame: resample empty");
        return;
    }
#else
    if (thiz == RT_NULL || thiz->speaker == RT_NULL || data == RT_NULL ||
        data_len == 0)
    {
        XZ_AUDIO_HOT_LOG("drop speaker frame: audio not ready");
        return;
    }
#endif
    int try_times = 0;
    while (!thiz->is_exit)
    {
#if PKG_XIAOZHI_USING_AEC
        ret = audio_write(thiz->speaker,
                          (uint8_t *)sifli_resample_get_output(thiz->resample),
                          bytes);
#else
        ret = audio_write(thiz->speaker, data, data_len);
#endif
        if (ret)
        {
            break;
        }
        rt_thread_mdelay(10);
        try_times++;
        if (try_times > 10)
        {
            XZ_AUDIO_HOT_LOG("speaker write failed len=%u tx=%d", data_len,
                             thiz->is_tx_enable);
            break;
        }
    }
}

static void xz_audio_kick_downlink_decode(xz_audio_t *thiz)
{
    rt_slist_t *decode;

    if (thiz == RT_NULL || thiz->event == RT_NULL || !thiz->is_tx_enable)
    {
        return;
    }

    rt_enter_critical();
    decode = rt_slist_first(&thiz->downlink_decode_busy);
    rt_exit_critical();

    if (decode != RT_NULL)
    {
        rt_event_send(thiz->event, XZ_EVENT_DOWNLINK);
    }
}

static void xz_audio_encode_and_send_pcm(xz_audio_t *thiz, uint32_t pcm_bytes,
                                         rt_bool_t zero_pad_tail)
{
    opus_int32 len;

    if (thiz == RT_NULL || thiz->encoder == RT_NULL || pcm_bytes == 0 ||
        pcm_bytes > XZ_MIC_FRAME_LEN)
    {
        XZ_AUDIO_HOT_LOG("drop mic frame: encoder not ready bytes=%u", pcm_bytes);
        return;
    }

    if (zero_pad_tail && pcm_bytes < XZ_MIC_FRAME_LEN)
    {
        rt_memset(&thiz->encode_in[pcm_bytes], 0, XZ_MIC_FRAME_LEN - pcm_bytes);
    }

    len = opus_encode(thiz->encoder, (const opus_int16 *)thiz->encode_in,
                      XZ_MIC_FRAME_LEN / 2, thiz->encode_out, XZ_MIC_FRAME_LEN);
    if (len < 0 || len > XZ_MIC_FRAME_LEN)
    {
        XZ_AUDIO_HOT_LOG("drop mic frame: opus_encode ret=%d samplerate=%d", len,
                         g_xz_context.sample_rate);
        return;
    }
#ifdef XIAOZHI_USING_MQTT
    xz_audio_send(thiz->encode_out, len);
#else
    xz_audio_send_using_websocket(thiz->encode_out, len);
#endif
}

static void xz_opus_thread_entry(void *p)
{
    int err;
    xz_audio_t *thiz = &xz_audio;
    thiz->decoder = opus_decoder_create(24000, 1, &err);
    if (thiz->decoder == RT_NULL)
    {
        LOG_E("opus_decoder_create failed err=%d", err);
        xz_audio_open_cleanup(thiz);
        goto exit;
    }

    thiz->encoder = opus_encoder_create(16000, 1, OPUS_APPLICATION_VOIP, &err);
    if (thiz->encoder == RT_NULL)
    {
        LOG_E("opus_encoder_create failed err=%d", err);
        xz_audio_open_cleanup(thiz);
        goto exit;
    }
    opus_encoder_ctl(thiz->encoder,
                     OPUS_SET_EXPERT_FRAME_DURATION(OPUS_FRAMESIZE_60_MS));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_VBR(1));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_VBR_CONSTRAINT(1));

    opus_encoder_ctl(thiz->encoder, OPUS_SET_BITRATE(8000));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_SIGNAL(OPUS_SIGNAL_VOICE));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_COMPLEXITY(0));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_LSB_DEPTH(16));

    opus_encoder_ctl(thiz->encoder, OPUS_SET_DTX(0));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_INBAND_FEC(0));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_PACKET_LOSS_PERC(0));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_PREDICTION_DISABLED(0));

    opus_encoder_ctl(thiz->encoder,
                     OPUS_SET_MAX_BANDWIDTH(OPUS_BANDWIDTH_WIDEBAND));
    opus_encoder_ctl(thiz->encoder, OPUS_SET_BANDWIDTH(OPUS_AUTO));
    
    opus_encoder_ctl(thiz->encoder, OPUS_SET_FORCE_MODE(MODE_SILK_ONLY));
    while (!thiz->is_exit)
    {
        rt_uint32_t evt = 0;
        rt_event_recv(thiz->event, XZ_EVENT_ALL,
                      RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                      rt_tick_from_millisecond(XZ_AUDIO_THREAD_EVENT_WAIT_MS),
                      &evt);
        if (thiz->is_exit)
        {
            break;
        }
        if (evt & XZ_EVENT_EXIT)
        {
            break;
        }
        if (evt & XZ_EVENT_SPK_CTRL)
        {
            if (g_speaker_target_on)
            {
                xz_speaker_open(thiz);
            }
            else
            {
                xz_speaker_close(thiz);
            }
        }
        if ((evt & XZ_EVENT_MIC_RX) && thiz->is_rx_enable)
        {
            rt_ringbuffer_get(thiz->rb_opus_encode_input,
                              (uint8_t *)&thiz->encode_in[0], XZ_MIC_FRAME_LEN);
            xz_audio_encode_and_send_pcm(thiz, XZ_MIC_FRAME_LEN, RT_FALSE);
            if (rt_ringbuffer_data_len(thiz->rb_opus_encode_input) >=
                XZ_MIC_FRAME_LEN)
            {
                rt_event_send(thiz->event, XZ_EVENT_MIC_RX);
            }
        }

        if (evt & XZ_EVENT_MIC_FLUSH)
        {
            uint32_t pending_bytes;

            while (rt_ringbuffer_data_len(thiz->rb_opus_encode_input) >=
                   XZ_MIC_FRAME_LEN)
            {
                rt_ringbuffer_get(thiz->rb_opus_encode_input,
                                  (uint8_t *)&thiz->encode_in[0],
                                  XZ_MIC_FRAME_LEN);
                xz_audio_encode_and_send_pcm(thiz, XZ_MIC_FRAME_LEN, RT_FALSE);
            }

            pending_bytes = rt_ringbuffer_data_len(thiz->rb_opus_encode_input);
            if (pending_bytes > 0)
            {
                rt_memset(thiz->encode_in, 0, sizeof(thiz->encode_in));
                rt_ringbuffer_get(thiz->rb_opus_encode_input,
                                  (uint8_t *)&thiz->encode_in[0],
                                  pending_bytes);
                xz_audio_encode_and_send_pcm(thiz, pending_bytes, RT_TRUE);
            }

            thiz->mic_rx_count = 0;
            if (g_xz_mic_flush_sem != RT_NULL)
            {
                rt_sem_release(g_xz_mic_flush_sem);
            }
        }

        if (evt & XZ_EVENT_SPK_TX)
        {
        }

        if ((evt & XZ_EVENT_DOWNLINK) && thiz->is_tx_enable)
        {
            rt_slist_t *decode;
            rt_enter_critical();
            decode = rt_slist_first(&thiz->downlink_decode_busy);
            rt_exit_critical();
            if (decode == RT_NULL)
            {
                XZ_AUDIO_HOT_LOG("drop stale downlink event");
                continue;
            }
            xz_decode_queue_t *queue =
                rt_container_of(decode, xz_decode_queue_t, node);
            opus_int32 res = opus_decode(
                thiz->decoder, (const uint8_t *)queue->data, queue->data_len,
                (opus_int16 *)&thiz->downlink_decode_out[0], XZ_SPK_FRAME_LEN,
                0);

            if (res != XZ_SPK_FRAME_LEN / 2)
            {
                XZ_AUDIO_HOT_LOG("decode out samples=%d", res);
            }
            if(res > 0)
            {
                audio_write_and_wait(thiz, (uint8_t *)thiz->downlink_decode_out,
                                 res * 2);
            }

            uint8_t need_decode_gain = 0;
            rt_enter_critical();
            rt_slist_remove(&thiz->downlink_decode_busy, decode);
            rt_slist_append(&thiz->downlink_decode_idle, decode);
            if (rt_slist_first(&thiz->downlink_decode_busy))
            {
                need_decode_gain = true;
            }
            rt_exit_critical();

            if (need_decode_gain)
                rt_event_send(thiz->event, XZ_EVENT_DOWNLINK);
        }
    }
exit:
    if (thiz->encoder)
    {
        opus_encoder_destroy(thiz->encoder);
        thiz->encoder = RT_NULL;
    }
    if (thiz->decoder)
    {
        opus_decoder_destroy(thiz->decoder);
        thiz->decoder = RT_NULL;
    }

    rt_kprintf("---xz thread exit---\r\n");
    if (g_xz_audio_thread_exit_sem != RT_NULL)
    {
        rt_sem_release(g_xz_audio_thread_exit_sem);
    }
}

void xz_aec_mic_open(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

    if (!thiz->mic)
    {
        LOG_I("mic on,thiz->inited=%d", thiz->inited);
        if (!thiz->inited || thiz->rb_opus_encode_input == RT_NULL)
        {
            LOG_E("aec mic open skipped: audio not ready");
            return;
        }
        // if (thiz->inited != 2)
        // {
        //     return;
        // }
        while (1)
        {
            uint8_t buf[128];
            rt_base_t level = rt_hw_interrupt_disable();  // 添加临界区保护
            int len = rt_ringbuffer_get(thiz->rb_opus_encode_input,
                                        (uint8_t *)&buf[0], sizeof(buf));
            rt_hw_interrupt_enable(level);  // 恢复中断
            if (len == 0)
            {
                break;
            }
        }
        audio_parameter_t pa = {0};
        pa.write_bits_per_sample = 16;
        pa.write_channnel_num = 1;
        pa.write_samplerate = 16000;
        pa.read_bits_per_sample = 16;
        pa.read_channnel_num = 1;
        pa.read_samplerate = 16000;
        pa.read_cache_size = 0;
        pa.write_cache_size = 32000;
        pa.is_need_3a = 1;
        thiz->mic = audio_open(AUDIO_TYPE_LOCAL_MUSIC, AUDIO_TXRX, &pa,
                               mic_callback, NULL);
        if (thiz->mic == RT_NULL)
        {
            LOG_E("aec mic open failed");
            thiz->speaker = RT_NULL;
            thiz->is_rx_enable = 0;
            thiz->is_tx_enable = 0;
            return;
        }
        thiz->speaker = thiz->mic;
        thiz->is_rx_enable = 1;   //麦克风常开
        thiz->is_tx_enable = 1;

    }
}
void xz_mic_open(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

#if !PKG_XIAOZHI_USING_AEC
    if (!thiz->mic)
    {
        LOG_I("mic on");
        if (!thiz->inited || thiz->rb_opus_encode_input == RT_NULL)
        {
            LOG_E("mic open skipped: audio not ready");
            return;
        }
        while (1)
        {
            uint8_t buf[128];
            int len = rt_ringbuffer_get(thiz->rb_opus_encode_input,
                                        (uint8_t *)&buf[0], sizeof(buf));
            if (len == 0)
            {
                break;
            }
        }

        audio_parameter_t pa = {0};
        pa.write_bits_per_sample = 16;
        pa.write_channnel_num = 1;
        pa.write_samplerate = 24000;
        pa.read_bits_per_sample = 16;
        pa.read_channnel_num = 1;
        pa.read_samplerate = 16000;
        pa.read_cache_size = 0;
        pa.write_cache_size = 0;
        thiz->mic = audio_open(AUDIO_TYPE_LOCAL_RECORD, AUDIO_RX, &pa,
                               mic_callback, NULL);
        if (thiz->mic == RT_NULL)
        {
            LOG_E("mic open failed");
            thiz->is_rx_enable = 0;
            return;
        }
        thiz->is_rx_enable = 1;
    }
#endif
}

void xz_aec_mic_close(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

    if (thiz->mic)
    {
        LOG_I("mic off");
        xz_audio_safe_close(&thiz->mic, "mic");
        thiz->is_rx_enable = 0;
    }
}
void xz_mic_close(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

#if !PKG_XIAOZHI_USING_AEC
    if (thiz->mic)
    {
        LOG_I("mic off");
        xz_audio_safe_close(&thiz->mic, "mic");
        thiz->is_rx_enable = 0;
    }
#endif
}

void xz_mic_stop_capture(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

    if (thiz->mic)
    {
        thiz->is_rx_enable = 0;
    }
}

rt_err_t xz_mic_flush_pending(xz_audio_t *thiz, rt_int32_t timeout_ms)
{
    rt_uint32_t pending_bytes;

    thiz = xz_audio_resolve(thiz);
    if (!thiz->inited || thiz->event == RT_NULL || thiz->encoder == RT_NULL)
    {
        return -RT_ERROR;
    }

    pending_bytes = rt_ringbuffer_data_len(thiz->rb_opus_encode_input);
    if (pending_bytes == 0 && thiz->mic_rx_count == 0)
    {
        return RT_EOK;
    }

    if (g_xz_mic_flush_sem == RT_NULL)
    {
        return -RT_ERROR;
    }

    while (rt_sem_take(g_xz_mic_flush_sem, 0) == RT_EOK)
    {
    }

    rt_event_send(thiz->event, XZ_EVENT_MIC_FLUSH);
    return rt_sem_take(g_xz_mic_flush_sem, rt_tick_from_millisecond(timeout_ms));
}

void xz_speaker_open(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);
    audio_server_select_private_audio_device(AUDIO_TYPE_LOCAL_MUSIC, AUDIO_DEVICE_SPEAKER);

#if PKG_XIAOZHI_USING_AEC
    #if STOP_SPEAKER_WHEN_DETECTED_MIC_VOICE
        if (thiz->speaker == RT_NULL)
        {
            LOG_E("speaker on skipped: audio not ready");
            thiz->is_tx_enable = 0;
            return;
        }
        LOG_I("speaker on");
        xiaozhi_ui_chat_status("\u8bb2\u8bdd\u4e2d...");
        thiz->is_tx_enable = 1;
        xz_audio_kick_downlink_decode(thiz);
    #endif
#else
    if (!thiz->speaker)
    {
        LOG_I("speaker on,thiz->inited=%d", thiz->inited);
        // if (thiz->inited != 2)
        // {
        //     return;
        // }
        xiaozhi_ui_chat_status("\u8bb2\u8bdd\u4e2d...");
        audio_parameter_t pa = {0};
        pa.write_bits_per_sample = 16;
        pa.write_channnel_num = 1;
        pa.write_samplerate = 24000;
        pa.read_bits_per_sample = 16;
        pa.read_channnel_num = 1;
        pa.read_samplerate = 16000;
        pa.read_cache_size = 0;
        /* 24K 单声道语音播放不需要 32KB 缓冲，缩小后可避免在高内存压力下断言。 */
        pa.write_cache_size = XZ_SPK_WRITE_CACHE_SIZE;
        thiz->speaker =
            audio_open2(AUDIO_TYPE_LOCAL_MUSIC, AUDIO_TX, &pa, NULL, NULL,
                        AUDIO_DEVICE_SPEAKER);
        if (thiz->speaker == RT_NULL)
        {
            LOG_E("speaker open failed");
            thiz->is_tx_enable = 0;
            g_speaker_target_on = 0U;
            return;
        }
        thiz->is_tx_enable = 1;
    }
    xz_audio_kick_downlink_decode(thiz);
#endif
}
void xz_speaker_close(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);

#if PKG_XIAOZHI_USING_AEC
    #if STOP_SPEAKER_WHEN_DETECTED_MIC_VOICE
    LOG_I("speaker off");
    xiaozhi_ui_chat_status("\u5f85\u547d\u4e2d...");
        thiz->is_tx_enable = 0;
        g_xz_downlink_drop_close_count +=
            xz_audio_move_downlink_busy_to_idle(thiz);
    #endif
#else

    if (thiz->speaker)
    {
        if (!xz_audio_wait_downlink_drain(thiz,
                                          XZ_SPK_CLOSE_DRAIN_TIMEOUT_MS))
        {
            g_xz_speaker_close_timeout_count++;
            XZ_AUDIO_HOT_LOG("speaker close drain timeout count=%u",
                             g_xz_speaker_close_timeout_count);
        }
        uint32_t cache_time_ms = 150;
        audio_ioctl(thiz->speaker, 1, &cache_time_ms);
        if (cache_time_ms > XZ_SPK_CLOSE_CACHE_WAIT_MAX_MS)
        {
            XZ_AUDIO_HOT_LOG("speaker cache wait clamp %u->%u",
                             cache_time_ms, XZ_SPK_CLOSE_CACHE_WAIT_MAX_MS);
            cache_time_ms = XZ_SPK_CLOSE_CACHE_WAIT_MAX_MS;
        }
        rt_thread_mdelay(cache_time_ms + XZ_SPK_CLOSE_CACHE_PAD_MS);
        xz_audio_safe_close(&thiz->speaker, "speaker");
        thiz->is_tx_enable = 0;
        uint32_t dropped = xz_audio_move_downlink_busy_to_idle(thiz);
        if (dropped > 0)
        {
            g_xz_downlink_drop_close_count += dropped;
            XZ_AUDIO_HOT_LOG("drop %u downlink packets on speaker close total=%u",
                             dropped, g_xz_downlink_drop_close_count);
        }
    }
#endif
}

void xz_speaker_abort(xz_audio_t *thiz)
{
    thiz = xz_audio_resolve(thiz);
    g_speaker_target_on = 0U;
    LOG_I("speaker abort");

    thiz->is_tx_enable = 0;
    g_xz_downlink_drop_close_count += xz_audio_move_downlink_busy_to_idle(thiz);

    if (thiz->speaker)
    {
        uint32_t cache_time_ms = 0;

        audio_ioctl(thiz->speaker, 1, &cache_time_ms);
        xz_audio_safe_close(&thiz->speaker, "speaker_abort");
    }
}

static uint32_t xz_downlink_queue_alloc_size(uint32_t packet_size)
{
    uint32_t required;
    uint32_t remainder;

    if (packet_size > UINT32_MAX - XZ_DOWNLINK_PACKET_HEADROOM)
    {
        return 0;
    }

    required = packet_size + XZ_DOWNLINK_PACKET_HEADROOM;
    if (required < XZ_DOWNLINK_QUEUE_INITIAL_SIZE)
    {
        required = XZ_DOWNLINK_QUEUE_INITIAL_SIZE;
    }

    remainder = required % XZ_DOWNLINK_QUEUE_GROW_ALIGN;
    if (remainder != 0)
    {
        uint32_t add = XZ_DOWNLINK_QUEUE_GROW_ALIGN - remainder;

        if (required <= UINT32_MAX - add)
        {
            required += add;
        }
    }

    return required;
}

static rt_err_t xz_downlink_queue_ensure_capacity(xz_decode_queue_t *queue,
                                                  uint32_t packet_size)
{
    uint32_t new_size;
    uint8_t *new_data;
    uint8_t *old_data;

    if (queue == RT_NULL ||
        packet_size > UINT32_MAX - XZ_DOWNLINK_PACKET_HEADROOM)
    {
        return -RT_ERROR;
    }

    if (queue->data != RT_NULL &&
        queue->size >= packet_size + XZ_DOWNLINK_PACKET_HEADROOM)
    {
        return RT_EOK;
    }

    new_size = xz_downlink_queue_alloc_size(packet_size);
    if (new_size == 0)
    {
        return -RT_ERROR;
    }

    new_data = opus_heap_malloc(new_size);
    if (new_data == RT_NULL)
    {
        LOG_E("drop downlink packet: alloc %u bytes failed, keep %u bytes",
              new_size, queue->size);
        return -RT_ENOMEM;
    }

    old_data = queue->data;
    queue->data = new_data;
    queue->size = new_size;

    if (old_data != RT_NULL)
    {
        opus_heap_free(old_data);
    }

    return RT_EOK;
}

/**
 * @brief 打开音频解码器和编码器
 *
 * 该函数负责初始化音频处理模块，包括事件创建、队列初始化、线程创建等
 * 如果模块尚未初始化，则根据参数决定是否使用WebSocket，并准备音频处理线程
 *
 * @param is_websocket 指示是否使用WebSocket的标志
 */
void xz_audio_decoder_encoder_open(uint8_t is_websocket)
{
    // 获取音频处理模块的实例
    xz_audio_t *thiz = &xz_audio;

    LOG_I("%s", XZ_AUDIO_VERSION);
    // 检查模块是否已经初始化，避免重复初始化
    if (!thiz->inited)
    {
        memset(thiz, 0, sizeof(xz_audio_t));
        //thiz->inited = 1; 
#if PKG_XIAOZHI_USING_AEC
        int ret;
        thiz->vad_enabled = true;
        audio_parameter_t pa = {0};
        pa.write_bits_per_sample = 16;
        pa.write_channnel_num = 1;
        pa.write_samplerate = 16000;
        pa.read_bits_per_sample = 16;
        pa.read_channnel_num = 1;
        pa.read_samplerate = 16000;
        pa.read_cache_size = 0;
        pa.write_cache_size = 32000;
        pa.is_need_3a = 1;
        pa.disable_uplink_agc = 1;
        thiz->mic = audio_open(AUDIO_TYPE_LOCAL_MUSIC, AUDIO_TXRX, &pa,
                               mic_callback, NULL);
        if (thiz->mic == RT_NULL)
        {
            LOG_E("aec audio open failed");
            xz_audio_open_cleanup(thiz);
            return;
        }

        mic_gain_decrease(4);

        thiz->speaker = thiz->mic;
        thiz->resample = sifli_resample_open(1, 24000, 16000);
        if (thiz->resample == RT_NULL)
        {
            LOG_E("resample open failed");
            xz_audio_open_cleanup(thiz);
            return;
        }
        ret = WebRtcVad_Create(&thiz->handle);
        if (ret != 0 || thiz->handle == RT_NULL)
        {
            LOG_E("vad create failed ret=%d", ret);
            xz_audio_open_cleanup(thiz);
            return;
        }
        ret = WebRtcVad_Init(thiz->handle);
        if (ret != 0)
        {
            LOG_E("vad init failed ret=%d", ret);
            xz_audio_open_cleanup(thiz);
            return;
        }
        ret = WebRtcVad_set_mode(thiz->handle, 3); // 0 ~ 3
        if (ret != 0)
        {
            LOG_E("vad set mode failed ret=%d", ret);
            xz_audio_open_cleanup(thiz);
            return;
        }

#endif
        // 根据参数设置是否使用WebSocket
        thiz->is_websocket = is_websocket;

        // 创建事件，用于音频处理中的同步
        thiz->event = rt_event_create("xzaudio", RT_IPC_FLAG_FIFO);
        if (thiz->event == RT_NULL)
        {
            LOG_E("create xzaudio event failed");
            xz_audio_open_cleanup(thiz);
            return;
        }
        if (g_xz_mic_flush_sem == RT_NULL)
        {
            g_xz_mic_flush_sem = rt_sem_create("xzmicf", 0, RT_IPC_FLAG_FIFO);
            if (g_xz_mic_flush_sem == RT_NULL)
            {
                LOG_E("create xzmicf sem failed");
                xz_audio_open_cleanup(thiz);
                return;
            }
        }
        if (g_xz_audio_thread_exit_sem == RT_NULL)
        {
            g_xz_audio_thread_exit_sem =
                rt_sem_create("xzthx", 0, RT_IPC_FLAG_FIFO);
            if (g_xz_audio_thread_exit_sem == RT_NULL)
            {
                LOG_E("create xzthx sem failed");
                xz_audio_open_cleanup(thiz);
                return;
            }
        }
        xz_audio_sem_drain(g_xz_audio_thread_exit_sem);

        // 初始化空闲和忙状态的解码队列
        rt_slist_init(&thiz->downlink_decode_idle);
        rt_slist_init(&thiz->downlink_decode_busy);

        // 为下行链路解码队列分配内存
        for (int i = 0; i < XZ_DOWNLINK_QUEUE_NUM; i++)
        {
            thiz->downlink_queue[i].size = XZ_DOWNLINK_QUEUE_INITIAL_SIZE;
            thiz->downlink_queue[i].data =
                opus_heap_malloc(thiz->downlink_queue[i].size);
            if (thiz->downlink_queue[i].data == RT_NULL)
            {
                LOG_E("alloc downlink queue[%d] failed: %u bytes", i,
                      thiz->downlink_queue[i].size);
                thiz->downlink_queue[i].size = 0;
            }

            // 将新分配的队列节点添加到空闲队列中
            rt_slist_append(&thiz->downlink_decode_idle,
                            &thiz->downlink_queue[i].node);
        }

#if PKG_XIAOZHI_USING_AEC
        // 使用 PSRAM 中的静态缓冲区创建 ringbuffer
        thiz->rb_vad_cache = (struct rt_ringbuffer *)rt_malloc(sizeof(struct rt_ringbuffer));
        if (thiz->rb_vad_cache == RT_NULL)
        {
            LOG_E("alloc vad ringbuffer failed");
            xz_audio_open_cleanup(thiz);
            return;
        }
        rt_ringbuffer_init(thiz->rb_vad_cache, g_rb_vad_cache_buf, RB_VAD_CACHE_SIZE);
        thiz->rb_opus_encode_input = (struct rt_ringbuffer *)rt_malloc(sizeof(struct rt_ringbuffer));
        if (thiz->rb_opus_encode_input == RT_NULL)
        {
            LOG_E("alloc opus ringbuffer failed");
            xz_audio_open_cleanup(thiz);
            return;
        }
        rt_ringbuffer_init(thiz->rb_opus_encode_input, g_rb_opus_encode_buf, RB_OPUS_ENCODE_SIZE);
#else
        // 创建用于编码输入的环形缓冲区，使用 PSRAM 静态缓冲区
        thiz->rb_opus_encode_input = (struct rt_ringbuffer *)rt_malloc(sizeof(struct rt_ringbuffer));
        if (thiz->rb_opus_encode_input == RT_NULL)
        {
            LOG_E("alloc opus ringbuffer failed");
            xz_audio_open_cleanup(thiz);
            return;
        }
        rt_ringbuffer_init(thiz->rb_opus_encode_input, g_rb_opus_encode_buf, RB_OPUS_ENCODE_SIZE);
#endif
        // 初始化音频处理线程
        rt_err_t err;
        err = rt_thread_init(&thiz->thread, XZ_THREAD_NAME,
                             xz_opus_thread_entry, // 音频处理线程入口函数
                             NULL, g_xz_opus_stack, sizeof(g_xz_opus_stack),
                             RT_THREAD_PRIORITY_MIDDLE +
                                 RT_THREAD_PRIORITY_HIGHER,
                             RT_THREAD_TICK_DEFAULT);
        if (err != RT_EOK)
        {
            LOG_E("init opus thread failed err=%d", err);
            xz_audio_open_cleanup(thiz);
            return;
        }

        // 启动音频处理线程
        thiz->inited = 1;
        err = rt_thread_startup(&thiz->thread);
        if (err != RT_EOK)
        {
            LOG_E("start opus thread failed err=%d", err);
            thiz->inited = 0;
            rt_thread_detach(&thiz->thread);
            xz_audio_open_cleanup(thiz);
            return;
        }

        // 标记模块已初始化
        if (!thiz->inited || thiz->is_exit)
        {
            LOG_E("opus thread init failed");
            return;
        }
#if PKG_XIAOZHI_USING_AEC
        thiz->is_rx_enable = 1;   //麦克风常开
        thiz->is_tx_enable = 1;
#endif
        
        rt_kprintf("xz_audio_decoder_encoder open ok\n");
    }
        rt_kprintf("xz_audio_decoder_encoder open2 ok\n");
}

void xz_audio_decoder_encoder_close(void)
{
    xz_audio_t *thiz = &xz_audio;
    audio_client_t mic_handle;
    audio_client_t speaker_handle;
    bool shared_handle;

    LOG_I("xz_audio_decoder_encoder close in  %d", thiz->inited);

    if (!thiz->inited)
    {
        return;
    }
    
    thiz->is_exit = 1;
    if (thiz->event != RT_NULL)
    {
        rt_event_send(thiz->event, XZ_EVENT_EXIT);
    }
    if (g_xz_audio_thread_exit_sem != RT_NULL &&
        rt_sem_take(g_xz_audio_thread_exit_sem,
                    rt_tick_from_millisecond(XZ_AUDIO_THREAD_EXIT_TIMEOUT_MS)) !=
            RT_EOK)
    {
        g_xz_audio_thread_exit_timeout_count++;
        LOG_E("wait thread %s exit timeout total=%u", XZ_THREAD_NAME,
              g_xz_audio_thread_exit_timeout_count);
    }

    // 释放 ringbuffer 结构体（缓冲区在 PSRAM 中静态分配，不需要释放）
    rt_free(thiz->rb_opus_encode_input);
    thiz->rb_opus_encode_input = NULL;
    if (thiz->event != RT_NULL)
    {
        rt_event_delete(thiz->event);
        thiz->event = RT_NULL;
    }

    xz_audio_release_downlink_queues(thiz);
#if PKG_XIAOZHI_USING_AEC
    // 释放 ringbuffer 结构体（缓冲区在 PSRAM 中静态分配，不需要释放）
    rt_free(thiz->rb_vad_cache);
    thiz->rb_vad_cache = NULL;
    sifli_resample_close(thiz->resample);
    mic_handle = thiz->mic;
    speaker_handle = thiz->speaker;
    shared_handle = (speaker_handle == mic_handle);
    thiz->mic = NULL;
    thiz->speaker = NULL;
    xz_audio_safe_close(&mic_handle, "mic");
    if (!shared_handle)
    {
        xz_audio_safe_close(&speaker_handle, "speaker");
    }
    thiz->rb_vad_cache = NULL;
    thiz->resample = NULL;
    if (thiz->handle)
    {
        WebRtcVad_Free(thiz->handle);
        thiz->handle = NULL;
    }
#else
    mic_handle = thiz->mic;
    speaker_handle = thiz->speaker;
    shared_handle = (speaker_handle == mic_handle);
    thiz->mic = NULL;
    thiz->speaker = NULL;
    xz_audio_safe_close(&mic_handle, "mic");
    if (!shared_handle)
    {
        xz_audio_safe_close(&speaker_handle, "speaker");
    }
#endif
    thiz->inited = 0;

    rt_kprintf("xz_audio_decoder_encoder close out ok\n");
}

void reinit_audio()
{
    xz_audio_t *thiz = &xz_audio;
    xz_aec_mic_close(thiz);
    xz_speaker_close(thiz);
    xz_audio_decoder_encoder_close();
    // 重新打开音频解码器和编码器
    xz_audio_decoder_encoder_open(1); 
    if (!thiz->inited)
    {
        LOG_E("reinit audio failed");
        return;
    }
    xz_aec_mic_open(thiz);
    xz_speaker_open(thiz);
}

void xz_audio_downlink(uint8_t *data, uint32_t size, uint32_t *aes_value,
                       uint8_t need_aes)
{
    xz_audio_t *thiz = &xz_audio;
    rt_slist_t *idle;
    if (!thiz->inited || thiz->event == RT_NULL)
    {
        XZ_AUDIO_HOT_LOG("%s invalid", __FUNCTION__);
        return;
    }
    if (data == RT_NULL || size == 0)
    {
        XZ_AUDIO_HOT_LOG("drop empty downlink packet size=%u", size);
        return;
    }
    // LOG_I("%s tx=%d inited=%d\r\n", __FUNCTION__, thiz->is_tx_enable,
    // thiz->inited);
    rt_enter_critical();
    idle = rt_slist_first(&thiz->downlink_decode_idle);
    rt_exit_critical();
    if (idle)
    {
        xz_decode_queue_t *queue =
            rt_container_of(idle, xz_decode_queue_t, node);
        if (xz_downlink_queue_ensure_capacity(queue, size) != RT_EOK)
        {
            return;
        }

        queue->data_len = size;
        if (need_aes)
        {
            HAL_AES_init((uint32_t *)&(g_xz_context.key[0]), 16, aes_value,
                         AES_MODE_CTR);
            HAL_AES_run(AES_DEC, data, queue->data, size);
        }
        else
        {
            memcpy(queue->data, data, size);
        }
        rt_enter_critical();
        rt_slist_remove(&thiz->downlink_decode_idle, idle);
        rt_slist_append(&thiz->downlink_decode_busy, idle);
        rt_exit_critical();

        rt_event_send(thiz->event, XZ_EVENT_DOWNLINK);
    }
    else
    {
        g_xz_downlink_drop_no_idle_count++;
        XZ_AUDIO_HOT_LOG("drop downlink no idle total=%u mic=%u speaker=%u",
                         g_xz_downlink_drop_no_idle_count,
                         (uint32_t)(uintptr_t)thiz->mic,
                         (uint32_t)(uintptr_t)thiz->speaker);
        rt_event_send(thiz->event, XZ_EVENT_DOWNLINK);
    }
}

/************************ (C) COPYRIGHT Sifli Technology *******END OF FILE****/
