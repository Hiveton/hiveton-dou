...
#define TOUCH_FW_VERSION_REG            (0xA6)
...
static rt_uint32_t cst816_i2c_speed_hz(void)
{
#ifdef BSP_USING_BOARD_SF32LB52_LCD_N16R8
    return 200000;
#else
...
static void cst816_correct_pos(touch_msg_t p_msg)
{
#ifdef BSP_USING_BOARD_SF32LB52_LCD_N16R8
    if (p_msg->x >= LCD_HOR_RES_MAX)
    {
        p_msg->x = (rt_uint16_t)(LCD_HOR_RES_MAX - 1U);
    }

    if (p_msg->y >= LCD_VER_RES_MAX)
    {
        p_msg->y = (rt_uint16_t)(LCD_VER_RES_MAX - 1U);
    }
#endif
}
...
static rt_err_t cst816_i2c_read_reg8_retry(uint8_t reg, uint8_t *p_data, uint8_t len,
                                           rt_uint8_t retry_count, rt_int32_t retry_delay_ms)
{
    ...
    {
        ret = cst816_i2c_read_reg8(reg, p_data, len);
        if (ret == RT_EOK)
        {
            return RT_EOK;
        }
...
static rt_err_t init(void)
{
    ...
    rt_thread_mdelay(20);
    fw_ret = cst816_i2c_read_reg8_retry(TOUCH_FW_VERSION_REG, &fw_ver, 1, 5, 8);
    id_ret = cst816_i2c_read_reg8_retry(FTS_REG_CHIP_ID, &id, 1, 5, 8);

    LOG_I("cst816 probe read fw_ret=%d fw=0x%x id_ret=%d id=0x%x irq=%d",
          fw_ret, fw_ver, id_ret, id, rt_pin_read(TOUCH_IRQ_PIN));
    ...
    {
        LOG_W("cst816 chip id is unavailable, continue with fw=0x%x id_ret=%d id=0x%x",
              fw_ver, id_ret, id);
    }
#endif
    /*
...
static rt_bool_t probe(void)
{
...

    {
        struct rt_i2c_configuration configuration =
        {
            ...
            .addr = 0,
            .timeout = 5000,
            .max_hz  = cst816_i2c_speed_hz(),
        };

        rt_i2c_configure(ft_bus, &configuration);
    }

    LOG_I("cst816 probe OK hz=%lu dma=%d", (unsigned long)cst816_i2c_speed_hz(), cst816_use_dma());
